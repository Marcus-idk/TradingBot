...
- Fixtures:
  ...
  - `polygon_settings` - Load PolygonSettings from the environment or skip when unavailable.
...
- Purpose: Live integration tests for Polygon providers using the real API.
- Tags: [async] [integration] [network]
- Tests:
  - `test_live_news_fetch` - Test fetching real news data from Polygon API.
  - `test_live_multiple_symbols` - Test fetching news for multiple symbols.
  - `test_live_error_handling` - Test error handling with invalid symbol.
...
- Purpose: PolygonSettings provider-specific tests (shared checks elsewhere).
- Tests: (none)
...
- Purpose: Shared env loading/validation across Finnhub, Polygon, OpenAI, Gemini.
- Tags: [shared]
- Fixtures:
...
- Fixtures:
  - `provider_spec_company` - Parametrized company-news provider spec for Finnhub and Polygon.
  - `provider_spec_macro` - Parametrized macro-news provider spec for Finnhub and Polygon.
  - `provider_spec_prices` - Price provider spec for Finnhub used by shared price contract tests.
  - `client_spec` - Parametrized HTTP client spec for Finnhub and Polygon clients.
- Helpers: `CompanyProviderSpec`, `MacroProviderSpec`, `PriceProviderSpec`, `ClientSpec`, `_finnhub_settings`, `_polygon_settings`
- Tests: (none)
...
- Purpose: Polygon-specific macro news tests (complement contract coverage).
- Tags: [async]
- Tests:
  **TestPolygonMacroNewsProvider**
  - `test_fetch_incremental_handles_pagination` - Test fetch incremental handles pagination.
  - `test_since_buffer_applied` - Test since buffer applied.
  - `test_empty_results_stops_pagination` - Test empty results stops pagination.
  - `test_fetch_incremental_missing_results_raises` - Missing results key is treated as a schema/contract error.
  - `test_next_url_without_cursor_stops_pagination` - Test next url without cursor stops pagination.
  - `test_extract_cursor_from_next_url` - Test extract cursor from next url.
  - `test_extract_cursor_exception_returns_none` - Test extract cursor exception returns none.
  - `test_non_dict_publisher_defaults_to_polygon` - Test non dict publisher defaults to polygon.

### `tests/unit/data/providers/test_polygon_news.py`
- Purpose: Polygon-specific company news tests (complement contract coverage).
- Tags: [async]
- Tests:
  **TestPolygonNewsProvider**
  - `test_fetch_incremental_handles_pagination` - Test fetch incremental handles pagination.
  - `test_extract_cursor_from_next_url` - Test extract cursor from next url.
  - `test_fetch_symbol_news_applies_buffer_filter` - Test fetch symbol news applies buffer filter.
  - `test_fetch_symbol_news_missing_results_raises` - Missing results key is treated as a schema/contract error.
...
- Tests:
  **TestIsoParsingHelpers**
  ...
  - `test_parse_rfc3339_handles_naive_as_utc` - Test parse rfc3339 handles naive as utc.
  - `test_parse_rfc3339_raises_for_non_string` - Test parse rfc3339 raises for non string.
  - `test_parse_rfc3339_invalid_format_raises` - Test parse rfc3339 invalid format raises.
...
- Tests:
  **TestParseRetryAfter**
  ...
  - `test_invalid_retry_after_values_log_debug_and_return_none` - Test graceful handling of malformed Retry-After headers with logging
...
- Tests:
  **TestBuildPlan**
  - `test_finnhub_macro_plan_uses_id_cursor` - Test finnhub macro plan uses id cursor.
  - `test_finnhub_company_plan_maps_each_symbol` - Test finnhub company plan maps each symbol.
  - `test_polygon_company_plan_bootstraps_symbols` - Test polygon company plan bootstraps symbols.
  - `test_polygon_company_plan_raises_on_bootstrap_detection_failure` - Polygon plan raises when bootstrap detection hits DB errors.
  - `test_reddit_social_plan_uses_per_symbol_map` - Reddit plan returns per-symbol cursors with first-run window.

  **TestCommitUpdates**
  - `test_symbol_scope_clamps_future` - Test symbol scope clamps future.
  - `test_global_scope_bootstrap_updates` - Test global scope bootstrap updates.
  - `test_id_scope_writes_last_fetched_max` - Test id scope writes last fetched max.
  - `test_id_scope_noop_without_last_fetched` - Test id scope noop without last fetched.
  - `test_social_scope_commits_per_symbol_and_clamps_future` - Reddit social watermarks track per symbol and clamp future.

  **TestHelpers**
  - `test_get_settings_missing_attribute_raises` - Test get settings missing attribute raises.
  - `test_is_macro_stream_matches_rule` - Test is macro stream matches rule.
