...
class TestClassifyUsSession:
    ...
    @pytest.mark.parametrize("ts_utc, expected", [
        # —— Regular (EDT, UTC-4) ——
        (datetime(2024, 7, 15, 13, 30, tzinfo=timezone.utc), Session.REG),  # 09:30 ET
        (datetime(2024, 7, 15, 19, 59, tzinfo=timezone.utc), Session.REG),  # 15:59 ET

        # —— Regular (EST, UTC-5) ——
        (datetime(2024, 1, 17, 14, 30, tzinfo=timezone.utc), Session.REG),  # 09:30 ET
        (datetime(2024, 1, 17, 20, 59, tzinfo=timezone.utc), Session.REG),  # 15:59 ET

        # —— Pre-market (EDT) ——
        (datetime(2024, 7, 15, 8, 0, tzinfo=timezone.utc),  Session.PRE),   # 04:00 ET
        (datetime(2024, 7, 15, 13, 29, tzinfo=timezone.utc), Session.PRE),  # 09:29 ET

        # —— Pre-market (EST) ——
        (datetime(2024, 1, 17, 9, 0, tzinfo=timezone.utc),  Session.PRE),   # 04:00 ET
        (datetime(2024, 1, 17, 14, 29, tzinfo=timezone.utc), Session.PRE),  # 09:29 ET

        # —— After-hours (EDT) ——
        (datetime(2024, 7, 15, 20, 0, tzinfo=timezone.utc), Session.POST),  # 16:00 ET
        (datetime(2024, 7, 15, 23, 59, tzinfo=timezone.utc), Session.POST), # 19:59 ET
        # After-hours (EDT) end is exclusive at 20:00 ET:
        (datetime(2024, 7, 16, 0, 0, tzinfo=timezone.utc),  Session.CLOSED),# 20:00 ET

        # —— After-hours (EST) ——
        (datetime(2024, 1, 17, 21, 0, tzinfo=timezone.utc), Session.POST),  # 16:00 ET
        (datetime(2024, 2, 15, 0, 59, tzinfo=timezone.utc), Session.POST),  # 19:59 ET
        # After-hours (EST) end is exclusive at 20:00 ET:
        (datetime(2024, 1, 18, 1, 0, tzinfo=timezone.utc),  Session.CLOSED),# 20:00 ET

        # —— Closed (overnight pre-PRE) ——
        (datetime(2024, 7, 15, 7, 59, tzinfo=timezone.utc), Session.CLOSED), # 03:59 ET
        (datetime(2024, 1, 17, 8, 59, tzinfo=timezone.utc), Session.CLOSED), # 03:59 ET
    ])
    ...
    def test_exact_boundaries_and_precision(self):
        ...
        # PRE start inclusive
        assert classify_us_session(datetime(2024, 7, 15, 8, 0, tzinfo=timezone.utc)) == Session.PRE

        # PRE end exclusive / REG start inclusive
        assert classify_us_session(datetime(2024, 7, 15, 13, 29, 59, 999999, tzinfo=timezone.utc)) == Session.PRE
        assert classify_us_session(datetime(2024, 7, 15, 13, 30, 0, 1, tzinfo=timezone.utc)) == Session.REG

        # POST start inclusive / POST end exclusive
        assert classify_us_session(datetime(2024, 7, 15, 20, 0, tzinfo=timezone.utc)) == Session.POST
        assert classify_us_session(datetime(2024, 7, 16, 0, 0, tzinfo=timezone.utc)) == Session.CLOSED
    ...
    def test_dst_transitions(self):
        ...
        assert classify_us_session(datetime(2024, 3, 11, 13, 30, tzinfo=timezone.utc)) == Session.REG
        # Fall back: Mon 2024-11-04 09:30 ET = 14:30Z
        assert classify_us_session(datetime(2024, 11, 4, 14, 30, tzinfo=timezone.utc)) == Session.REG

    def test_weekends_holidays_early_closes(self):
        """Test weekends, major holidays, and early close days."""
        # Weekends → CLOSED
        assert classify_us_session(datetime(2024, 7, 13, 14, 0, tzinfo=timezone.utc)) == Session.CLOSED  # Sat
        assert classify_us_session(datetime(2024, 7, 14, 14, 0, tzinfo=timezone.utc)) == Session.CLOSED  # Sun

        # Major holidays → CLOSED
        for d in [(2024,12,25), (2024,11,28), (2024,7,4), (2024,1,1), (2024,1,15), (2024,9,2)]:
            assert classify_us_session(datetime(*d, 14, 0, tzinfo=timezone.utc)) == Session.CLOSED

        # Early close (Black Friday 2024-11-29): 13:00 ET close
        assert classify_us_session(datetime(2024, 11, 29, 17, 30, tzinfo=timezone.utc)) == Session.REG   # 12:30 ET
        assert classify_us_session(datetime(2024, 11, 29, 18, 30, tzinfo=timezone.utc)) == Session.POST  # 13:30 ET

        # Christmas Eve early close (2024-12-24)
        assert classify_us_session(datetime(2024, 12, 24, 17, 30, tzinfo=timezone.utc)) == Session.REG   # 12:30 ET
        assert classify_us_session(datetime(2024, 12, 24, 19, 0,  tzinfo=timezone.utc)) == Session.POST  # 14:00 ET
