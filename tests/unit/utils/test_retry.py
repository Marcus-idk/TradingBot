...
class TestParseRetryAfter:
    ...
    def test_invalid_retry_after_values_log_debug_and_return_none(self, caplog):
        ...
        assert any(
            "Invalid Retry-After header 'garbage'" in record.message for record in caplog.records
        )
...
class TestRetryAndCall:
    ...
    async def test_exponential_backoff_with_jitter(self):
        ...
        op = AsyncMock(
            side_effect=[RetryableError("error 1"), RetryableError("error 2"), "success"]
        )

        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            result = await retry_and_call(op, attempts=3, base=0.5, mult=2.0, jitter=0.1)
