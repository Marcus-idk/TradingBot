...
def cleanup_sqlite_artifacts(db_path: str) -> None:
    ...
    """
    # If the DB was never created, nothing to do
    if not os.path.exists(db_path):
        return
    
    # First GC: flush any lingering cursors/connections from test code
    gc.collect()
    
    # Checkpoint WAL and switch to DELETE mode to release Windows locks
    try:
        uri = f"file:{db_path}?mode=rw"  # Read-write mode, don't create if missing
        ...
        # DB already gone/corrupt? Just proceed to file deletion
        pass
    
    # Second GC: ensure the cleanup connection/cursors are finalized
    gc.collect()
    
    # Delete files in Windows-safe order: WAL artifacts first, then main DB
    for suffix in ("-wal", "-shm", ""):
        try:
            ...
            # Windows still has a lock - try once more after GC, then give up
            gc.collect()
            try:
                os.remove(db_path + suffix)
            except (FileNotFoundError, PermissionError):
                pass  # Don't fail tests over cleanup issues
...
class TestNewsItemStorage:
    ...
    def test_store_news_deduplication_insert_or_ignore(self):
        ...
        try:
            ...
            store_news_items(db_path, items)
    ...
    def test_store_news_empty_list_no_error(self):
        ...
        try:
            init_database(db_path)
            store_news_items(db_path, [])  # Should not raise error
...
class TestPriceDataStorage:
    ...
    def test_store_price_data_type_conversions(self):
        ...
        try:
            ...
            # Store price data
            store_price_data(db_path, items)
    ...
    def test_store_price_data_deduplication(self):
...
class TestAnalysisResultUpsert:
    ...
    def test_upsert_analysis_conflict_resolution(self):
        ...
        try:
            ...
            upsert_analysis_result(db_path, result1)
            ...
            upsert_analysis_result(db_path, result2)
    ...
    def test_upsert_analysis_auto_created_at(self):
        ...
        try:
            ...
            # Store result
            upsert_analysis_result(db_path, result)
...
class TestHoldingsUpsert:
    ...
    def test_upsert_holdings_timestamp_handling(self):
        ...
        try:
            ...
            upsert_holdings(db_path, holdings1)
            ...
            upsert_holdings(db_path, holdings2)
    ...
    def test_upsert_holdings_auto_timestamps(self):
        ...
        try:
            ...
            # Store holdings
            upsert_holdings(db_path, holdings)
...
class TestQueryOperations:
    ...
    def test_get_news_since_timestamp_filtering(self):
        ...
        try:
            ...
            results = get_news_since(db_path, since)
    ...
    def test_get_price_data_since_ordering(self):
        ...
        try:
            ...
            results = get_price_data_since(db_path, since)
    ...
    def test_get_all_holdings_ordering(self):
        ...
        try:
            ...
            # Query all holdings
            results = get_all_holdings(db_path)
    ...
    def test_get_analysis_results_symbol_filtering(self):
        ...
        try:
            ...
            aapl_results = get_analysis_results(db_path, symbol="AAPL")
            ...
            all_results = get_analysis_results(db_path)
...
class TestErrorHandling:
    ...
    def test_query_operations_with_empty_database(self):
        ...
        try:
            ...
            assert get_news_since(db_path, datetime.now(timezone.utc)) == []
            assert get_price_data_since(db_path, datetime.now(timezone.utc)) == []
            assert get_all_holdings(db_path) == []
            assert get_analysis_results(db_path) == []
            assert get_analysis_results(db_path, symbol="NONEXISTENT") == []
